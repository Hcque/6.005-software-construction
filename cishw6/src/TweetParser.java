...
public class TweetParser {
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
    	...
    	List<String> senten = sentenceSplit(s);
    	List<List<String>> result = new LinkedList<>();
    	for (String ss: senten) {
    		result.add(parseAndCleanSentence(ss));
    	}
    	return result;
    ...
    static List<String> csvFileToTweets(String pathToCSVFile, int tweetColumn) {
    	FileLineIterator itr = new FileLineIterator(pathToCSVFile);
    	List<String> result = new LinkedList<>();
    	while (itr.hasNext()) {
    		String tw = extractColumn(itr.next(), tweetColumn);
			result.add(tw);
    	}
    ...
    public static List<List<String>> csvFileToTrainingData(String pathToCSVFile, int tweetColumn) {
    	List<String> L = csvFileToTweets(pathToCSVFile, tweetColumn);
		List<List<String>> result = new LinkedList<>();
    	for (String tw: L) {
			 result.addAll(parseAndCleanTweet(tw));
    	}
        return result; // todo: Complete this method
    }
